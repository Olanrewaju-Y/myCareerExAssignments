...
// 2. Return an array of drug names in lowercase.
  ...
  const drugNameInLowerCase = drugs.map(drug => drug.name.toLocaleLowerCase());
  console.log(drugNameInLowerCase);

// 3. Write a function that accepts a category and returns all drugs under that category.

function rtnDrugsInCategory(cat) {

 return drugsInCategory = drugs.filter(drug => drug.category.toLocaleLowerCase() === cat.toLocaleLowerCase());
  //  return drugsInCategory;
}
console.log(rtnDrugsInCategory("aNtIbiotIC"));

// 4. Log each drug’s name and its manufacturer.

    const drugName = drugs.map(drug => "name: " + drug.name + ", " + "manufacturer: " + drug.manufacturer)
...
// 6. Return a new array, each item should follow the format: "Drug: [name] - [dosageMg]mg".
   ...
   const nameNDosageMg = drugs.map(drug => "Drug: " + drug.name + " - " + drug.dosageMg + "mg");
   console.log(nameNDosageMg);

// 7. Write a function that returns all drugs with a stock less than 50.

  function less50Drugs() {
   return drugStock = drugs.filter(drug => drug.stock < 50)
    // return drugStock;
  }
  console.log(less50Drugs());

// 8. Return all drugs that are not prescription-only.

   const notPrescription = drugs.filter(drug => drug.isPrescriptionOnly !== true)
...
// 9. Write a function that takes a manufacturer name and returns how many drugs are from that company.
    ...
    function drugsFromManu(manu) {
    const drugsCountFromManu = drugs.filter(drug => drug.manufacturer === manu)
    console.log(drugsCountFromManu);
     const totalStock = drugsCountFromManu.reduce((sum, drugCountFromManu) => sum + drugCountFromManu.stock, 0);
        console.log(totalStock);
    }
    drugsFromManu("Pfizer")

// 10. Use forEach() to count how many drugs are Analgesics.

const dCR = drugs.forEach((drug) => {
